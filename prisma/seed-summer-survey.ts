...
export async function seedSummerSurvey(prisma: PrismaClient) {
  ...
  const surveyQuestions = [
    ...
    { label: "수련회 중 제일 좋았던 프로그램은?", type: "radio", isRequired: true, sortOrder: 4, options: JSON.stringify(SUMMER_PROGRAMS) },
    { label: "좋았던 이유는?", type: "textarea", isRequired: true, sortOrder: 5 },
    { label: "수련회 중 아쉬웠던 프로그램은?", type: "radio", isRequired: true, sortOrder: 6, options: JSON.stringify(SUMMER_PROGRAMS) },
